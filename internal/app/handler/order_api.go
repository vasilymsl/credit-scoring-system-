...
func (h *Handler) GetOrdersAPI(c *gin.Context) {
	...
	var priceMin *int
	if priceStr := c.Query("price_min"); priceStr != "" {
	...
	var priceMax *int
	if priceStr := c.Query("price_max"); priceStr != "" {
		if p, err := strconv.Atoi(priceStr); err == nil {
			priceMax = &p
		}
	}
	...
	orders, err := h.Repository.GetOrdersWithFilter(titleFilter, dateFrom, dateTo, priceMin, priceMax)
	if err != nil {
