...
func (r *Repository) GetOrdersWithFilter(titleFilter string, dateFrom, dateTo *time.Time, priceMin, priceMax *int) ([]ds.Order, error) {
	var orders []ds.Order
	query := r.db
	...
	// Фильтрация по цене (sum_from)
	if priceMin != nil {
		query = query.Where("sum_from >= ?", priceMin)
	}
	if priceMax != nil {
		query = query.Where("sum_from <= ?", priceMax)
	}
...
func (r *Repository) UpdateOrderImage(orderID int, imageURL string) error {
	return r.db.Model(&ds.Order{}).Where("id = ?", orderID).Update("image_url", imageURL).Error
}
